...
from .forms import TeacherLoginform,registerform,submitselectgradeform,Teacherselectgradeform,Addgradeselectform,Selectmajorform
from django.contrib import auth
# Create your views here.
from django.http import HttpResponse
from .models import Task,Student,Grade,abc,Teacher,Question,Answer,ID,Score,Thread
...
def teacher_b(request):
    ...
    if request.method =="POST":
        ...
        count = Question.objects.filter(question_who=username,question_grade=Grade).count()
        if count == 0:
...
def teacher_d(request):
    ...
    if request.method =="POST":
        ...
        a=Question.objects.filter(question_who=username,question_grade=Grade)
        a.delete()
...
def student_c(request):
    ...
    if request.method == "POST":
        ...
        if d == 0:
            ...
            b=Answer.objects.filter(question=cc)
            th1 = Thread.objects.filter(thread_question=cc)
            BB=list()
            for B in b:
                CC=B.thread_set.all()
                BB.append(CC)
            return render(request,"Student/头脑风暴/Tou-选择班级3.html",{"grade":a,"question":c,"answer":b,"thread":BB})
#学生头脑风暴-提交问题成功页面
...
def Submith(request):
    ...
    if request.method == "POST":
        a=request.POST.get("answer","")
        A=Answer.objects.get(answer=a)
        thread=request.POST.get("th1","None")
        question = request.POST.get("question","None")
        questions = Question.objects.get(question=question)
        # print(a)
        # print(thread)
        # print(question)
        Thread.objects.create(thread_question=questions,thread=thread,thread_who=username,thread_answer=A)
    return render(request,"Student/头脑风暴/Tou-选择班级.html")

# 提交Thread观点-老师
def TeacherSubmith(request):
    users = request.user
    username=ID.objects.get(username=users)
    if request.method =="POST":
        a=request.POST.get("answer","None")
        b=request.POST.get("th1","None")
        c=request.POST.get("question","None")
        A=Answer.objects.get(answer=a)
        C=Question.objects.get(question=c)
        Thread.objects.create(thread_question=C,thread=b,thread_who=username,thread_answer=A)
    return render(request,"Teacher/头脑风暴/回答发布成功.html")
...
def AddSee(request):
    ...
    if request.method =="POST":
        a=request.POST.get("answer","None")
        b=request.POST.get("th1","None")
        A=Answer.objects.filter(answer=a).update(answer_teacher=b)
    return render(request,"Teacher/头脑风暴/回答发布成功.html")
